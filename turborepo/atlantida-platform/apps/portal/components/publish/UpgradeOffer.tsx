...
const OFFER_DURATION_SEC = 24 * 60 * 60 // 24 horas
...
export function UpgradeOffer({ userId }: { userId: string }) {
    ...
    useEffect(() => {
        ...
        if (!seen) {
            ...
            const remaining = OFFER_DURATION_SEC - elapsed
            if (remaining > 0) {
    ...
    const handleUpgradeClick = () => {
        trackEvent.upgradeOfferClicked({
            userId,
            discount: '20_percent',
            source: 'publish_limit',
        })
    }
            ...
            <div className="flex flex-wrap justify-between items-start gap-4">
                <div>
                    ...
                    <p className="text-xs opacity-80 mt-1">
                        Válida por {formatTime(timeLeft)} más.
                    </p>
                </div>
                <Link
