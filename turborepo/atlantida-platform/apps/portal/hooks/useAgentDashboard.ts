...
export function useAgentDashboard() {
    const { user, updateUserProfile: authUpdateProfile } = useAuth()
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                setProfileHours(data.officeHours || "")
    ...
    const updateProfile = async (data: { name: string, phone: string, agency: string, hours: string, photo: string }) => {
        if (!user || !db) return
        setIsUpdatingProfile(true)
        try {
            // Update Auth Profile
            if (data.name !== user.displayName || data.photo !== user.photoURL) {
                await authUpdateProfile({ displayName: data.name, photoURL: data.photo })
            }
            ...
            if (userSnap.exists()) {
                ...
                await setDoc(userRef, { ...updateData, createdAt: new Date().toISOString(), role: "agent" })
            }

            // Local state update
            setProfileName(data.name)
            setProfilePhone(data.phone)
            setProfileAgency(data.agency)
