...
export function NearbyPlacesCard({ places, hasCoordinates }: NearbyPlacesCardProps) {
    ...
    // Siempre mostrar la tarjeta como diferencial; si no hay coordenadas o no hay datos, mostrar mensaje
    if (!hasCoordinates) {
        return (
            <motion.div
    ...
    const byCategory = places.reduce<Record<string, Poi[]>>((acc, poi) => {
        ...
        if (!acc[key]) acc[key] = []
        if (acc[key].length < 2) acc[key].push(poi) // hasta 2 por categoría en la tarjeta
        return acc
    }, {})
