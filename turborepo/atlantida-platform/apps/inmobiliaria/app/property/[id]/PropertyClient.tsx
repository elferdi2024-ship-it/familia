...
export default function PropertyClient({
  ...
  const [leadMessage, setLeadMessage] = useState(
    "Hola, estoy interesado en recibir más información sobre esta propiedad...",
  );
  const [isSubmittingLead, setIsSubmittingLead] = useState(false);
  const [leadSuccess, setLeadSuccess] = useState(false);
  const [showPhone, setShowPhone] = useState(false);
  const [contactType, setContactType] = useState<"contact" | "visit">(
    "contact",
  ); // "contact" or "visit"
  ...
  // Chat interno (consultar por este inmueble — requiere auth)
  const { user } = useAuth();
  ...
  const headerOpacity = useTransform(scrollY, [0, 500], [1, 0.4]);
  ...
  const openChat = async () => {
    if (!db || !property?.id || !property?.userId) return;
    if (!user) {
    ...
    setChatLoading(true);
    try {
      const convId = await getOrCreateConversation(db, {
  ...
  useEffect(() => {
    if (!user || !showAuthForChat || !db || !property?.id || !property?.userId) return;
    let cancelled = false;
    (async () => {
      try {
        const convId = await getOrCreateConversation(db, {
  ...
  }, [user, showAuthForChat, property?.id, property?.userId, property?.title]);
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const web3FormsData = {
        ...
        message: leadMessage,
        tipo_consulta:
          contactType === "visit" ? "Solicitud de Visita" : "Contacto General",
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-background pb-24 lg:pb-0 font-sans">
      {/* Gallery Section */}
      <section className="relative w-full max-w-7xl mx-auto px-4 lg:px-8 pt-24 pb-6 lg:pt-28">
        {/* Back and Share buttons float above or near the carousel */}
        <div className="absolute top-28 left-8 lg:top-36 lg:left-16 z-40 flex gap-3">
          <Link
            href="/search"
            className="w-10 h-10 bg-white/90 dark:bg-black/60 backdrop-blur rounded-full flex items-center justify-center text-slate-900 dark:text-white shadow-md hover:scale-105 active:scale-95 transition-all border border-slate-200 dark:border-white/10"
          >
            <span className="material-icons flex items-center justify-center">arrow_back</span>
          </Link>
        </div>

        <div className="absolute top-28 right-8 lg:top-36 lg:right-16 z-40 flex gap-3 flex-col sm:flex-row">
          <button
            onClick={shareProperty}
            className="w-10 h-10 bg-white/90 dark:bg-black/60 backdrop-blur rounded-full flex items-center justify-center text-slate-900 dark:text-white shadow-md hover:scale-105 active:scale-95 transition-all border border-slate-200 dark:border-white/10"
          >
            <span className="material-icons text-xl flex items-center justify-center">share</span>
          </button>
          <FavoriteButton
            propertyId={property.id}
            className="w-10 h-10 bg-white/90 dark:bg-black/60 shadow-md text-slate-900 border border-slate-200 dark:border-white/10 dark:text-white rounded-full flex items-center justify-center backdrop-blur hover:bg-white dark:hover:bg-black hover:scale-105 active:scale-95"
          />
        </div>

        <PropertyCarousel images={property.images} altText={property.title} />
      </section>

      <main className="max-w-7xl mx-auto px-6 lg:px-8 pb-12 relative z-10 bg-white dark:bg-background">
          ...
          >
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2">
                ...
                {property.badge && (
                  <span className="px-3 py-1 bg-green-500 text-white text-xs font-bold rounded-full uppercase tracking-wider animate-pulse">
                    {property.badge}
                  </span>
                )}
              </div>
              <h1 className="text-3xl lg:text-5xl font-extrabold text-slate-900 dark:text-white leading-tight tracking-tight">
                {property.title}
              </h1>
            ...
            <div className="lg:hidden bg-white dark:bg-card border border-slate-100 dark:border-white/10 backdrop-blur-md rounded-[2rem] p-6 shadow-sm flex items-center justify-between">
              ...
              {property.gastosComunes && (
                <div className="text-right border-l border-slate-100 dark:border-slate-800 pl-6">
                  ...
                  <p className="text-sm font-bold text-slate-600 dark:text-slate-400">
                    ${property.gastosComunes.toLocaleString()}{" "}
                    <span className="text-[9px]">UYU</span>
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div
                  key={i}
                  className="p-4 bg-slate-50 dark:bg-card rounded-2xl border border-slate-100 dark:border-white/10 flex flex-col items-center text-center gap-2 transition-colors hover:dark:bg-slate-800"
                >
            ...
            {/* Amenities */}
            <div className="p-8 bg-slate-50 dark:bg-card rounded-3xl border border-slate-100 dark:border-white/10">
              <h3 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-10 text-center">
                Amenidades y Servicios
              </h3>
              <div className="space-y-12">
                {Object.entries(AMENITIES_BY_CATEGORY).map(
                  ([category, items]) => {
                    // Filter category items to show only those present in the property
                    const propertyAmenities = items.filter((item) =>
                    ...
                    return (
                      <div key={category} className="space-y-6">
                        <div className="flex items-center gap-3">
                          ...
                          <h4 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 whitespace-nowrap">
                            {category}
                          </h4>
                          <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800"></div>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                          {propertyAmenities.map((amenity, i) => (
                            ...
                            >
                              <div className="w-10 h-10 rounded-[12px] bg-white dark:bg-card shadow-sm border border-slate-100 dark:border-white/10 flex items-center justify-center group-hover:scale-110 group-hover:border-primary/30 transition-all duration-300">
                                <span className="material-icons text-primary/80 group-hover:text-primary transition-colors text-lg">
                                  {amenity.icon}
                                </span>
                              </div>
                              <span className="text-slate-700 dark:text-slate-300 font-bold text-sm group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
                                {amenity.name}
                              </span>
                            </div>
                          ))}
            ...
            {(property.operation === "Alquiler" ||
              property.operation === "Alquiler Temporal") && (
                <div className="p-8 bg-slate-50 dark:bg-card rounded-3xl border border-slate-100 dark:border-white/10">
                  <h3 className="text-2xl font-serif font-bold tracking-tight text-slate-900 dark:text-white mb-6 text-center">
                    Garantías Aceptadas
                  </h3>
                  {property.acceptedGuarantees &&
                    property.acceptedGuarantees.length > 0 ? (
                    <div className="flex flex-wrap justify-center gap-3">
                      {property.acceptedGuarantees.map((guarantee, i) => (
                        <div
                          key={i}
                          className="px-6 py-3 bg-white dark:bg-card border border-slate-200 dark:border-white/10 rounded-xl text-slate-700 dark:text-slate-300 font-bold shadow-sm flex items-center gap-2"
                        >
                    ...
                    <p className="text-center text-slate-500 font-medium">
                      Consultar garantías aceptadas con el agente.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-2xl font-serif font-bold tracking-tight text-slate-900 dark:text-white">
                  Ubicación y Entorno
                </h3>
            ...
            {/* Floorplan Section */}
            <div className="space-y-6">
              <h3 className="text-2xl font-serif font-bold tracking-tight text-slate-900 dark:text-white text-center">
          ...
          <div className="hidden lg:block w-[400px]">
            ...
            >
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden group">
                ...
                <div className="relative z-10 space-y-6">
                  <div>
                    ...
                    {property.gastosComunes && (
                      <p className="mt-2 text-slate-500 font-medium">
                        Gastos Comunes: $
                        {property.gastosComunes.toLocaleString()} UYU
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-primary text-white font-bold rounded-xl transition-all flex items-center justify-center gap-3 shadow-lg shadow-primary/20 hover:opacity-90 active:scale-[0.98] disabled:opacity-50"
                    >
                ...
                <div className="my-8 flex items-center gap-4 p-4 rounded-xl bg-slate-50 dark:bg-card border border-slate-100 dark:border-white/10 hover:shadow-md transition-all">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">
                      {agentInfo?.agencyName || "Agente Verificado"} •{" "}
                      {showPhone
                        ? agentInfo?.phone || property.agentPhone
                        : "Teléfono oculto"}
                    </p>
                    {!showPhone && (
                      <button
                  ...
                  {leadSuccess ? (
                    <div className="bg-green-50 border border-green-100 rounded-xl p-6 text-center animate-in zoom-in-95 duration-300">
                      ...
                      <p className="text-sm text-green-700 mt-1">
                        El agente se contactará contigo a la brevedad.
                      </p>
                    ...
                    <form onSubmit={handleLeadSubmit} className="space-y-4">
                      ...
                      <div>
                        <label htmlFor="lead-name-desktop" className="sr-only">
                          Nombre completo
                        </label>
                        <input
                      ...
                      <div>
                        <label htmlFor="lead-email-desktop" className="sr-only">
                          Email de contacto
                        </label>
                        <input
                      ...
                      <div>
                        <label htmlFor="lead-msg-desktop" className="sr-only">
                          Mensaje
                        </label>
                        <textarea
                          id="lead-msg-desktop"
                          ...
                          placeholder={
                            contactType === "visit"
                              ? "Me gustaría visitar la propiedad..."
                              : "Consulta..."
                          }
                          rows={3}
                          required
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/20 rounded-2xl p-6">
                  <div className="flex items-start gap-4">
                    <span className="material-icons text-primary underline-offset-4">
                      insights
                    </span>
                    <div>
                      <p className="text-xs font-bold text-primary mb-1 uppercase tracking-widest">
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-400 leading-snug">
                        Este inmueble tiene un precio por m² un{" "}
                        <span className="font-bold text-slate-900 dark:text-white">
                          12% menor
                        </span>{" "}
                        a propiedades similares en la zona.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 gap-3">
          ...
          <div className="flex gap-2 flex-shrink-0">
            ...
            <button
              ...
              onClick={() => openChat()}
              disabled={chatLoading}
              className="h-12 px-4 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold border border-slate-200 dark:border-slate-700 active:scale-95 transition-transform disabled:opacity-50 flex items-center gap-1.5"
            >
      ...
      {/* Auth para chat (si no está logueado) */}
      {showAuthForChat && (
        <AuthModal
          open={showAuthForChat}
          onOpenChange={(open) => !open && setShowAuthForChat(false)}
          title="Iniciar chat"
          subtitle="Iniciá sesión o registrate para chatear por este inmueble."
        />
      )}

      {/* Modal chat */}
      <Dialog open={showChatModal} onOpenChange={(open) => !open && (setShowChatModal(false), setChatConvId(null))}>
        <DialogContent className="rounded-lg border border-slate-200 dark:border-slate-700 shadow-xl p-0 overflow-hidden max-w-lg max-h-[85vh] flex flex-col">
          {chatConvId && user && (
            <ConversationThread
              ...
              otherUserName={agentInfo?.displayName || "Agente"}
              propertyTitle={property.title}
              isLead={true}
          ...
          <div className="mt-auto bg-white dark:bg-slate-900 rounded-t-3xl relative z-10 p-6 pb-8 safe-bottom animate-in slide-in-from-bottom duration-300">
            ...
            {leadSuccess ? (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800/20 rounded-xl p-8 text-center">
                <span className="material-icons text-green-500 text-5xl mb-4">
                  check_circle
                </span>
                <h4 className="text-xl font-bold text-green-900 dark:text-green-400">
                  ¡Consulta Enviada!
                </h4>
                <p className="text-sm text-green-700 dark:text-green-500/80 mt-2">
                  Pronto recibirás una respuesta en tu email.
                </p>
