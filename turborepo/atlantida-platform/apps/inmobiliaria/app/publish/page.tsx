...
import { PublishStep1Schema } from "@repo/lib/validations"
...
function PublishPageContent() {
    ...
    // Cargar cantidad de propiedades para mostrar tarjeta de upgrade a usuarios free que ya tienen 1
    useEffect(() => {
    ...
    }, [user?.uid, userData?.plan, isEditing])
            ...
            <div className="min-h-screen pt-32 pb-20 px-6 flex flex-col items-center justify-center font-display bg-background-light dark:bg-background-dark text-slate-900 dark:text-white">
                <div className="max-w-md w-full text-center space-y-6">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-lg mb-8">
                        Para publicar una propiedad en MiBarrio.uy necesitas tener una cuenta. Es gratis y te tomará menos de 1 minuto.
                    </p>
                    <button
        ...
        <div className="font-display bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen flex flex-col pt-24 md:pt-28">
            ...
            <main className="flex-grow py-12 px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    {/* Tarjeta de upgrade cuando usuario free ya tiene 1 propiedad */}
                    {!isEditing && (userData?.plan || 'free') === 'free' && propertyCount !== null && propertyCount >= 1 && user && (
                        <>
                                    ...
                                    <div className="w-full h-[350px] md:h-[450px] rounded-lg overflow-hidden border border-slate-200 dark:border-slate-800 shadow-inner">
                                        <LocationPicker
                                            ...
                                            onAddressFound={(addressData) => {
                                                const updates: Record<string, string> = { address: addressData.address };
                                                ...
                                                if (addressData.department) {
                                                    ...
                                                    if (matchedDept) {
                                                        ...
                                                        if (addressData.city) {
                                                            const departmentData = (geoData as Record<string, Record<string, string[]>>)[matchedDept];
                                                            const cities = Object.keys(departmentData || {});
                                                            ...
                                                            if (matchedCity) {
                                                                ...
                                                                if (addressData.neighborhood) {
                                                                    const neighborhoods = departmentData[matchedCity] || [];
