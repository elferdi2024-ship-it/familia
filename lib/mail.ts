...
export async function sendLeadEmail(to: string, leadData: LeadEmailData) {
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: 'Barrio.uy <onboarding@resend.dev>',
            to: [to],
            ...
            html: `
                <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
                    <div style="background: linear-gradient(135deg, #0a4ecd 0%, #1e40af 100%); padding: 32px 24px; border-radius: 12px 12px 0 0;">
                        ...
                        <p style="color: #93c5fd; margin: 8px 0 0; font-size: 14px;">Barrio.uy — Plataforma Inmobiliaria Uruguay</p>
                    </div>
                    ...
                    <div style="padding: 24px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
                        ...
                        <div style="margin-top: 24px; text-align: center;">
                            ${leadData.leadPhone ? `<a href="https://wa.me/${leadData.leadPhone.replace(/\D/g, '')}?text=Hola ${leadData.leadName}, vi tu consulta por ${leadData.propertyTitle} en Barrio.uy." style="background-color: #25d366; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: 700; font-size: 14px; margin-right: 8px;">Responder por WhatsApp</a>` : ''}
                            <a href="https://barrio.uy/my-properties/leads" style="background-color: #0a4ecd; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: 700; font-size: 14px;">Ver en Dashboard</a>
                        </div>
